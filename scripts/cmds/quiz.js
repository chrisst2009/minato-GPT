...
const BASE_URL = 'https://quixweb.onrender.com/api';
...
module.exports = {
  ...
  async syncUserProfile(api, userId) {
    try {
      const userInfo = await api.getUserInfo(userId);
      const info = userInfo[userId] || {};

      const name = info.name || info.firstName || "Anonymous Player";
      ...
      let gender = "";
      if (info.gender === 2) gender = "male";
      else if (info.gender === 1) gender = "female";

      const payload = { userId, name, profilePicUrl };
      if (gender) payload.gender = gender;

      axios.post(`${BASE_URL}/user/update`, payload).catch(() => {});

      return name;

    } catch (error) {
      console.warn("syncUserProfile failed for", userId, error?.message || error);
      return "Anonymous Player";
    }
  },
  ...
  onStart: async function ({ message, event, args, commandName, getLang, api, usersData }) {
    try {
      ...
      switch (command) {
        ...
        case "leaderboard":
        case "lb":
          return await this.handleLeaderboard(message, getLang, args.slice(1), api, event);
        case "category":
          if (args.length > 1) {
  ...
  async handleRank(message, event, getLang, api, usersData) {
    try {
      const userName = await this.syncUserProfile(api, event.senderID);
  ...
  async handleLeaderboard(message, getLang, args, api, event) {
    try {
      if (event?.senderID) this.syncUserProfile(api, event.senderID).catch(() => {});
  ...
  async handleTrueOrFalse(message, event, commandName, api) {
    try {
      this.syncUserProfile(api, event.senderID).catch(() => {});
 ...
 onReaction: async function ({ message, event, Reaction, api, usersData }) {
    try {
      ...
      const userName = await this.syncUserProfile(api, event.userID);
      ...
      try {
        ...
        if (isCorrect) {
          const torfStreak = user.currentStreak || 1;
          const totalMoneyReward = Math.min(torfStreak * 1000, 100000);
          ...
          const successMsg = `${randomTorfMsg}\n` +
            ...
            `💰 𝗠𝗼𝗻𝗲𝘆 𝗘𝗮𝗿𝗻𝗲𝗱: +${totalMoneyReward.toLocaleString()} 💎 ${torfStreak >= 100 ? '🏆 MAX!' : `(🔥 Streak ${torfStreak})`}\n` +
  ...
  onReply: async function ({ message, event, Reply, getLang, api, usersData }) {
    ...
    try {
      ...
      if (result === "correct") {
        ...
        const totalMoneyReward = Math.min(currentStreak * 1000, 100000);

        userData.money = (userData.money || 0) + totalMoneyReward;
        await usersData.set(event.senderID, userData);

        const difficultyBonus = Reply.difficulty === 'hard' ? ' 🔥' : Reply.difficulty === 'easy' ? ' ⭐' : '';
        const streakBonus2 = currentStreak >= 5 ? ` 🚀 ${currentStreak}x streak!` : '';
        const flagBonus = Reply.isFlag ? ' 🏁' : '';
        const animeBonus = Reply.isAnime ? ' 🎌' : '';
        const dailyBonus = Reply.isDailyChallenge ? ' 🌟' : '';

        responseMsg = `🎉 Correct! 💰\n` +
          `💵 Money: +${totalMoneyReward.toLocaleString()} ${currentStreak >= 100 ? '🏆 MAX!' : `(🔥 Streak ${currentStreak})`}\n` +
          `✨ XP: +${user.xpGained || 15}\n` +
  ...
  envConfig: {
    rewardPerStreak: 1000,
    maxReward: 100000,
    achievementReward: 50000,
    flagReward: 10000,
    animeReward: 10000,
    dailyChallengeBonus: 10000,
    hardDifficultyReward: 10000,
    easyDifficultyReward: 10000
  }
};
